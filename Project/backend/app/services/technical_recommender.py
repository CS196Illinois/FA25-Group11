...
class TechnicalRecommender:
    ...
    def warmup(self) -> None:
        ...
        logger.info("[TechnicalRecommender] Starting warmup - preloading data...")
        start = time.time()
        
        try:
            t0 = time.time()
            self._load_prereq_graph()
            logger.info(f"[TechnicalRecommender] Loaded prereq graph in {time.time() - t0:.2f}s")
            
            t0 = time.time()
            self._load_postreq_graph()
            logger.info(f"[TechnicalRecommender] Loaded postreq graph in {time.time() - t0:.2f}s")
            
            t0 = time.time()
            self._load_majors_data()
            logger.info(f"[TechnicalRecommender] Loaded majors data in {time.time() - t0:.2f}s")
            
            t0 = time.time()
            self._load_course_data()
            logger.info(f"[TechnicalRecommender] Loaded course data in {time.time() - t0:.2f}s")
            
            logger.info(f"[TechnicalRecommender] Warmup completed in {time.time() - start:.2f}s")
        except Exception as e:
            logger.error(f"[TechnicalRecommender] Warmup failed: {e}", exc_info=True)
